...
export function AddOneDriveFiles({ onBack, onAdd }) {
  ...
  const checkStatus = useCallback(async () => {
    try {
      setLoading(true);
      const res = await api.get("/oauth-connect/onedrive/status");
  ...
  const fetchFiles = useCallback(
    async (pageToken = null) => {
      ...
      try {
        ...
        const res = await api.get(`/oauth-connect/onedrive/files?${params}`);
  ...
  const handleConnect = async () => {
    try {
      const res = await api.get("/oauth-connect/onedrive/auth");
      window.location.href = res.data.data.authUrl;
    } catch (err) {
      toast.error(err.response?.data?.message || "Failed to start OneDrive connection");
    }
  };
  ...
  const handleDisconnect = async () => {
    try {
      ...
      await api.delete("/oauth-connect/onedrive/disconnect");
    } catch {
  ...
  const handleImport = async () => {
    ...
    try {
      setImporting(true);
      const res = await api.post("/oauth-connect/onedrive/import", {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden p-6 sm:p-8">
        <h2 className="mb-1 text-xl font-bold text-slate-800">OneDrive</h2>
          ...
          <div className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-200 bg-white py-16 text-center">
            <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-blue-50">
              <svg className="h-7 w-7" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path d="M10.09 4.79c.67-.2 1.37-.3 2.09-.29 2.82.05 5.28 1.81 6.34 4.31a5.5 5.5 0 0 1-1.02 10.69H6.5a5 5 0 0 1-1.68-9.71 6.5 6.5 0 0 1 5.27-5z" fill="#0364B8" />
                <path d="M10.09 4.79a6.5 6.5 0 0 1 5.27 4.01l.14.4H17.5a5.5 5.5 0 0 1 0 11H6.5a5 5 0 0 1-1.68-9.71" fill="#0078D4" opacity="0.8" />
              </svg>
            </div>
            <h3 className="mb-2 font-semibold text-slate-800">Connect your OneDrive</h3>
            <p className="mb-6 max-w-sm text-sm text-slate-500">
              Sign in with Microsoft to browse and import files from your OneDrive. You only need to do this once.
            </p>
