...
export const ZohoSalesIQIntegration = {
  key: "ZOHO_SALES_IQ",
  name: "Zoho SalesIQ",
  url: "www.zoho.com",
  connectionType: "manual",
  twoStep: true,
  connectButtonLabel: "Next",
      ...
      <div className="rounded-lg border border-purple-100 bg-purple-50 p-3">
        ...
        <ol className="text-[11px] text-purple-700 list-decimal pl-4 space-y-0.5">
          ...
          <li>
            ...
            <strong>When visitor clicks the widget</strong> → Next
          </li>
          <li>
            On the webhook URL screen, <strong>click Next below</strong> to get your callback URL
          </li>
        </ol>
      </div>
    );
  ...
  renderPostConnectContent: (extraData, { copiedKey, setCopiedKey }) => {
    ...
    const webhookUrl = `${webhookBaseUrl}/api/webhooks/zoho-salesiq?token=${integrationId}`;

    return (
      <div className="flex flex-col gap-3">
        <p className="text-[13px] text-slate-600">
          Copy the callback URL below and paste it into the Zobot webhook URL field, then click Done in Zoho.
        </p>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => {
                ...
                setCopiedKey("zoho-webhook-url");
                setTimeout(() => setCopiedKey(null), 2000);
              }}
              className="shrink-0 rounded p-1 hover:bg-slate-200"
            >
              {copiedKey === "zoho-webhook-url" ? (
